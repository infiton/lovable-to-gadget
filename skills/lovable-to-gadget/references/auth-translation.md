...
const [{ fetching }, signIn] = useAction(api.user.signIn);
const [{ fetching }, signUp] = useAction(api.user.signUp);
...
const VerifyEmail = () => {
  ...
  // Render verifying/success/error states with navigation back to app
};
```
